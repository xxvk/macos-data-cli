...
URL_VALUE="https://example.invalid/macos-data/recurrence-test/$SUFFIX"
...
jq -n --arg start "$START" --arg end "$END" --arg url "$URL_VALUE" --arg suffix "$SUFFIX" '{
  title: ("macos-data disposable recurrence " + $suffix),
  startDate: $start,
  endDate: $end,
  timeZone: "Asia/Tokyo",
  url: $url,
...
printf '%s' '{"title":"macos-data detached recurrence occurrence"}' >"$TMP_DIR/this-patch.json"
...
jq -e 'length == 6 and .[1].title == "macos-data detached recurrence occurrence" and .[0].title != .[1].title and .[2].title != .[1].title' "$TMP_DIR/after-this.json" >/dev/null
FOURTH_ID="$(jq -r '.[3].id' "$TMP_DIR/after-this.json")"

printf '%s' '{"title":"macos-data future recurrence occurrences"}' >"$TMP_DIR/future-patch.json"
...
jq -e --arg fourth "$FOURTH_START" 'length == 6 and all(.[] | select(.startDate >= $fourth); .title == "macos-data future recurrence occurrences")' "$TMP_DIR/after-future.json" >/dev/null
