...
| macOS 26.x + 其他 `V*` 或未知 schema | 禁用直接读取，使用有上限的 Mail.app fallback |
| macOS 26.x + `V10` 但没有 Full Disk Access | 提示 FDA；有 Automation 时使用有限 Mail.app fallback |
...
1. **元数据查询：**索引 SQLite -> 仅在条件无索引时做有时间上限的 SQLite scan ->
   仅当 SQLite 不可用时才使用有限 Mail.app 查询。SQLite 返回完整空结果后不再回退。
...
可复现的非 UI 本机 release gate 已在 macOS 26.4 通过：60 个测试、Release build、
