...
During local development, use the Debug binary after running `swift build`:

```text
.build/debug/macos-data
```

Skills should allow the path to be configured with `MACOS_DATA_CLI`; they must
