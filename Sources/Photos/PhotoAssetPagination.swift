...
public enum PhotoAssetPagination {
    private static let prefix = "photoassetpage_"

    public static func page(
        items: [PhotoAssetPayload],
        query: PhotoAssetQuery,
        complete: Bool
    ) throws -> PagedResult<PhotoAssetPayload> {
        guard (1...Pagination.maximumLimit).contains(query.limit) else { throw PaginationError.invalidLimit }
        let fingerprint = digest([
            "photo-assets-v1",
            ISO8601DateFormatter().string(from: query.start),
            ISO8601DateFormatter().string(from: query.end),
            query.albumID ?? "-",
            query.mediaType?.rawValue ?? "-",
            query.favorite.map(String.init) ?? "-",
            String(query.includeHidden),
            String(query.includeLocation)
        ].joined(separator: ":"))
    ...
    private static func startIndex(_ cursor: String?, items: [PhotoAssetPayload], fingerprint: String) throws -> Int {
        ...
        guard parts.count == 3,
              parts[0] == "photo-assets-v1",
              parts[1] == Substring(fingerprint),
    ...
    private static func encode(id: String, fingerprint: String) -> String {
        prefix + Data("photo-assets-v1:\(fingerprint):\(digest(id))".utf8).base64EncodedString()
            .replacingOccurrences(of: "+", with: "-")
